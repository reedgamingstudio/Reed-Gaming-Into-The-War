...
public class ConnectMenu : Photon.MonoBehaviour  {
	...
	void OnGUI () {
		...
		//Main Menu
		GUILayout.BeginArea(new Rect (Screen.width/2 - 250, Screen.height/2 - 150, 500, 330), "Into The War", GUI.skin.GetStyle("window"));
			ShowConnectMenu();
		GUILayout.EndArea();
	...
	void ShowConnectMenu(){
		...
		if(!createRoom){
			...
			GUILayout.BeginHorizontal();
				...
				ExitGames.Client.Photon.Hashtable setMapName = new ExitGames.Client.Photon.Hashtable(); 
