...
public class RoomMultiplayerMenu : Photon.MonoBehaviour {
    ...
    public GameObject OCPlayerPrefab;
    ...
    System.Collections.IEnumerator RoundEnded(){
		gameMode = (string)PhotonNetwork.room.customProperties["GameMode"];
		if(gameMode == "TDM"){
	...
	void SpawnPlayer(string teamName){
		...
		if(teamName == team_1.teamName){
			if(gameMode == "TDM"){
				temp = Random.Range(0, team_1.spawnPoints.Count-1);
            ...
            {
                //Spawn player in OC mode
                temp = Random.Range(0, allSpawnPoints.Count-1);
                Player = PhotonNetwork.Instantiate(OCPlayerPrefab.name, allSpawnPoints[temp].position, allSpawnPoints[temp].rotation, 0);
                Player.name = PhotonNetwork.player.name;
            }
            else
            {
			...
			temp = Random.Range(0, team_2.spawnPoints.Count-1);
    ...
    System.Collections.IEnumerator LoadMap(string sceneName){
