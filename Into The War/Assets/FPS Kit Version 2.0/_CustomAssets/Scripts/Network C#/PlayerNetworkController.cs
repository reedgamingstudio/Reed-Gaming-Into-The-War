    ...
    void Update(){
        if (!photonView.isMine){
			...
			if(MovementAnimation != currentAnimation){
				...
				//Set animation speed
				if(animType == "Walking"){
					headLookController.animation[MovementAnimation].speed = animationSpeed.walkSpeed;
				}
				if(animType == "Running"){
					headLookController.animation[MovementAnimation].speed = animationSpeed.runSpeed;
				}
				if(animType == "Crouch"){
					headLookController.animation[MovementAnimation].speed = animationSpeed.crouchSpeed;
				}
				if(animType == "Prone"){
					headLookController.animation[MovementAnimation].speed = animationSpeed.proneSpeed;
				}
				
				//Set animation layer and wrap mode
				if(headLookController.animation[MovementAnimation] != null){
					headLookController.animation[MovementAnimation].layer = 1;
					headLookController.animation[MovementAnimation].wrapMode = WrapMode.Loop;
				}
			}
			if(headLookController.animation[MovementAnimation] != null){
				headLookController.animation.CrossFade(MovementAnimation);
			}
			...
			if(currentBlendedAnimation != blendedAnimation){
				...
				if(!mixedAnimations.Contains(currentBlendedAnimation) && currentBlendedAnimation != "Null" && headLookController.animation[currentBlendedAnimation] != null){
			   		headLookController.animation[currentBlendedAnimation].layer = 4;
					headLookController.animation[currentBlendedAnimation].wrapMode = WrapMode.Loop;
					headLookController.animation[currentBlendedAnimation].AddMixingTransform(shoulderR);
					headLookController.animation[currentBlendedAnimation].AddMixingTransform(shoulderL);	
					mixedAnimations.Add (currentBlendedAnimation);
				}
			}
			if(currentBlendedAnimation != "Null" && headLookController.animation[currentBlendedAnimation] != null){
				headLookController.animation.Play(currentBlendedAnimation); 
			}
			//**********************************************************************************************************
